...
export class KreditService {
  ...
  async addCollateral(creditId: number, data: any, userId: number) {
    try {
      ...
      if (isNaN(marketVal) || isNaN(assessedVal))
        throw new BadRequestException('Invalid market or assessed value');
      if (marketVal < 0 || assessedVal < 0)
        throw new BadRequestException('Values must be positive');
      ...
      throw new BadRequestException(
        `Failed to add collateral: ${error.message}`,
      );
    }
  }
  ...
  async activateCredit(creditId: number, data: any, userId: number) {
    ...
    if (!credit || credit.status !== 'APPROVE') {
      throw new BadRequestException(
        'Credit application is not approved or not found',
      );
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      try {
        ...
        if (!mapping) {
          throw new BadRequestException(
            'Konfigurasi COA Mapping untuk [KREDIT_REALISASI] belum tersedia. Hubungi Administrator.',
          );
        }
        ...
        const journalNo = await this.accountingService.generateJournalNumber(
          new Date(),
        );
        ...
        if (error instanceof Prisma.PrismaClientKnownRequestError) {
          if (error.code === 'P2003') {
            ...
            throw new BadRequestException(
              'Terjadi kesalahan data referensi (Akun Akuntansi tidak ditemukan). Harap periksa konfigurasi Chart of Account.',
            );
          }
        }
        ...
        throw new BadRequestException(
          `Gagal merealisasikan kredit: ${error.message}`,
        );
      }
    });
  }
    ...
    return this.prisma
      .$transaction(async (tx) => {
        ...
        if (totalPrincipalPaid + totalInterestPaid > 0) {
          ...
          const journalNo =
            await this.accountingService.generateJournalNumber(paymentDate);
