...
export class ProductConfigService {
  ...
  async updateProduct(code: string, data: any) {
    ...
    if (data.minInterestRate && data.maxInterestRate) {
      ...
      if (min > max) {
        throw new BadRequestException(
          'Min interest rate cannot exceed max interest rate',
        );
      }
    }
  ...
  async reorderProducts(
    order: { productCode: string; displayOrder: number }[],
  ) {
   ...
   */
  async hasExistingAccounts(
    code: string,
  ): Promise<{ hasAccounts: boolean; count: number }> {
    const product = await this.getProductByCode(code);
