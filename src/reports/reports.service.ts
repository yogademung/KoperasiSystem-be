...
export class ReportsService {
    ...
    return {
      ...
      pengajuan: {
        ...
        terbilang: this.terbilang(
          this.normalizeCurrency(credit.nominalPengajuan),
        ),
  ...
  async getCreditAgreementData(id: number) {
    ...
    return {
      ...
      pinjaman: {
        ...
        angsuranPokok:
          this.normalizeCurrency(credit.nominalPengajuan) /
          Number(credit.mohonJangkaWaktu), // Simplified
      },
  ...
  async getSavingsPassbookData(
    ...
    if (type === 'TABUNGAN') {
      ...
      const productConfig =
        await this.productConfigService.getProductByCode('TABRELA');
      ...
      // Fetch dynamic product name
      const productConfig =
        await this.productConfigService.getProductByCode('BRAHMACARI');
      title =
      ...
      // Fetch dynamic product name
      const productConfig =
        await this.productConfigService.getProductByCode('BALIMESARI');
      title =
      ...
      // Fetch dynamic product name
      const productConfig =
        await this.productConfigService.getProductByCode('WANAPRASTA');
      title =
      ...
      // Fetch dynamic product name
      const productConfig =
        await this.productConfigService.getProductByCode('ANGGOTA');
  ...
  async getDepositoCertificateData(noJangka: string) {
    ...
    return {
      ...
      detail: {
        ...
        perpanjanganOtomatis:
          deposito.payoutMode === 'ROLLOVER'
  ...
  async getAnggotaRegistrationData(accountNumber: string) {
    ...
    return {
      ...
      account: {
        ...
        terbilangPokok: this.terbilang(
          this.normalizeCurrency(account.principal),
        ),
        terbilangWajib: this.terbilang(
          this.normalizeCurrency(account.mandatoryInit),
        ),
      },
      nasabah: {
  ...
  async getAnggotaClosureData(accountNumber: string) {
    ...
    const adminTx = account.transactions.find(
      (t) => t.transType === 'BIAYA_ADMIN',
    );
    ...
    const productConfig =
      await this.productConfigService.getProductByCode('ANGGOTA');
    const productName =
      productConfig?.productName?.toUpperCase() || 'SIMPANAN ANGGOTA';
  ...
  async getTabrelaClosureData(noTab: string) {
    ...
    const productConfig =
      await this.productConfigService.getProductByCode('TABRELA');
    const productName =
      productConfig?.productName?.toUpperCase() || 'TABUNGAN RELA';
  ...
  async getBukuBesar(
    ...
    if (accounts.length === 0) {
      throw new NotFoundException(
        `No accounts found in range ${fromAccount} - ${endAccount}`,
      );
    }
  ...
  async generateBukuBesarPDF(
    fromAccount: string,
    toAccount: string,
    startDate: string,
    endDate: string,
  ) {
    const data = await this.getBukuBesar(
      fromAccount,
      toAccount,
      startDate,
      endDate,
  ...
  async getTabrelaReceiptData(accountNumber: string) {
    return this.getGenericReceiptData(
      'nasabahTab',
      'noTab',
      accountNumber,
      'Tabrela',
  ...
  async getCollectorKPI(startDate: Date, endDate: Date) {
    ...
    return {
      ...
      summary: {
        ...
        totalMembersRegistered: kpiData.reduce(
          (sum, c) => sum + c.metrics.membersRegistered,
          0,
        ),
        totalTransactions: kpiData.reduce(
          (sum, c) => sum + c.metrics.transactionStats.totalTransactions,
          0,
        ),
        totalAmount: kpiData.reduce(
          (sum, c) => sum + c.metrics.transactionStats.totalAmount,
          0,
        ),
      },
    };
  ...
  async exportCoaPdf(): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      accounts.forEach((acc) => {
        ...
        doc.text(
          acc.accountName.substring(0, 40),
          startX + colWidths[0],
          currentY,
        );
        doc.text(
