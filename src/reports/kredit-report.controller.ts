...
export class KreditReportController {
  ...
  async downloadKolektibilitasPDF(
    ...
    const pdf =
      await this.kreditReportService.generateKolektibilitasPDF(period);
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=Kolektibilitas_${period}.pdf`,
    );
    res.send(pdf);
  }
  ...
  async downloadDaftarKreditExcel(
    ...
    const excel =
      await this.kreditReportService.generateDaftarKreditExcel(status);
    res.setHeader(
      'Content-Type',
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=Daftar_Kredit_${status || 'ALL'}.xlsx`,
    );
    res.send(excel);
  }
  ...
  async downloadTunggakanPDF(
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=Tunggakan_${asOf}.pdf`,
    );
    res.send(pdf);
  }
}
