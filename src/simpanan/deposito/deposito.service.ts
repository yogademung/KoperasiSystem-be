...
export class DepositoService {
  ...
  async create(createDto: CreateDepositoDto, userId: number) {
    const { nasabahId, nominal, jangkaWaktuBulan, bunga, keterangan } =
      createDto;
  ...
  async withdraw(
    noJangka: string,
    userId: number,
    options?: { penalty?: number; adminFee?: number; reason?: string },
  ) {
    const deposito = await this.prisma.nasabahJangka.findUnique({
      where: { noJangka },
    ...
    if (deposito.status !== 'A')
      throw new BadRequestException('Deposito is not active');
