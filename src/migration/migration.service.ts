...
export class MigrationService {
  ...
  async previewNasabah(fileBuffer: Buffer) {
    try {
      ...
      worksheet.eachRow((row, idx) => {
        ...
        try {
          const tanggalLahirVal: any = row.getCell(7).value;
          ...
          const nasabahData = {
            id: globalId++,
            rowNumber: idx, // Use idx for excel row number
            excelRow: idx,
            nama,
            noKtp,
      ...
      console.log(
        `✅ Preview complete: ${summary.total} rows, ${summary.valid} valid, ${summary.duplicates} duplicates, ${summary.errors} errors`,
      );
  ...
  async confirmNasabah(validatedData: any[]) {
    try {
      console.log(
        `💾 Confirming upload of ${validatedData.length} nasabah records...`,
      );
      ...
      const nasabahToCreate = validatedData.map((n) => ({
        ...
        jenisKelamin:
          n.jenisKelamin === 'L' || n.jenisKelamin === 'P'
            ? n.jenisKelamin
            : null,
      ...
      console.log(
        `✅ Successfully created ${nasabahToCreate.length} nasabah records`,
      );
    ...
    try {
      ...
      worksheet.eachRow((row, rowNumber) => {
        ...
        try {
          const nama = row.getCell(1).text;
          const noKtp = row.getCell(2).text;
          const tanggalLahirVal: any = row.getCell(7).value; // type can be string, date, object
      ...
      console.log(
        `📊 Parsed ${nasabahList.length} nasabah from ${rowCount} rows`,
      );
      ...
      const validNasabah = nasabahList.filter(
        (n) => !existingKtps.has(n.noKtp),
      );

      console.log(
        `✅ Found ${existing.length} existing, ${validNasabah.length} new nasabah`,
      );
      ...
      if (validNasabah.length > 0) {
        ...
        console.log(
          `✅ Successfully created ${validNasabah.length} nasabah records`,
        );
      }
  ...
  async uploadAnggotaTransaction(fileBuffer: Buffer) {
    ...
    await workbook.xlsx.load(fileBuffer as any);
    const worksheet =
      workbook.getWorksheet('Transaksi Anggota') || workbook.getWorksheet(2);
    ...
    worksheet.eachRow((row, rowNumber) => {
      ...
      try {
        ...
        const tglDaftarVal: any = row.getCell(3).value;
    ...
    for (const txn of txns) {
      ...
      try {
        ...
        console.error(
          `Error creating anggota account for ${txn.noAnggota}:`,
          e,
        );
      }
    }
  ...
  async previewAnggota(fileBuffer: Buffer, redenominate: boolean = false) {
    ...
    const existingAnggotaSet = new Set(
      existingAnggota.map((a) => a.accountNumber),
    );
    ...
    const aggregates = Array.from(nasabahAggregates.entries()).map(
      ([id, stats]) => ({
        nasabahId: id,
        ...stats,
      }),
  ...
  async previewCoa(fileBuffer: Buffer, sheetMode: string = 'all') {
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.load(fileBuffer as any);

    const previewData: any[] = [];
    let globalId = 1;
    const accountCodesInFile = new Set<string>();

    // 1. Get Settings (Format)
    ...
    const regexPattern =
      '^' + coaFormat.replace(/[.-]/g, '\\$&').replace(/x/gi, '\\d') + '$';
    const coaRegex = new RegExp(regexPattern);

    // 2. Fetch Existing Accounts for duplicate check & parent lookup
    ...
    const existingAccountMap = new Map(
      existingAccounts.map((a) => [a.accountCode, a]),
    );

    // Helper: Format Code
    const formatAccountCode = (rawCode: string, format: string): string => {
      if (!rawCode || !/^\d+$/.test(rawCode)) return rawCode;
    ...
    // 3. Process Sheet 1 (Parent / Header)
    const parentSheet = workbook.getWorksheet(1);
    const parentMapInFile = new Map<string, { type: string; dbOrCr: string }>();
    ...
    if (parentSheet && processSheet1) {
      parentSheet.eachRow((row, rowNumber) => {
        ...
        // Validate Parent
        if (!accountCode) rowData.errors.push('Kode Akun wajib diisi');
        if (!accountName) rowData.errors.push('Nama Akun wajib diisi');
        if (!accountType) rowData.errors.push('Tipe Akun wajib diisi');
        if (!dbOrCr) rowData.errors.push('D/C wajib diisi');
    ...
    // 4. Process Sheet 2 (Transaction / Detail)
    ...
    if (childSheet && processSheet2) {
      childSheet.eachRow((row, rowNumber) => {
        ...
        if (!accountCode) rowData.errors.push('Kode Akun wajib diisi');
        if (!accountName) rowData.errors.push('Nama Akun wajib diisi');
        if (!parentCode) rowData.errors.push('Kode Induk wajib diisi');
        else if (!accountType) {
          rowData.errors.push(
            'Kode Induk tidak ditemukan (Tipe & D/C tidak bisa ditentukan)',
          );
        }
