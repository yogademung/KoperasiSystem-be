...
export class TemplateService {
  ...
  private validateTemplateSchema(schema: any): void {
    if (!schema || typeof schema !== 'object') {
      throw new BadRequestException(
        'Invalid template schema: must be an object',
      );
    }
    ...
    if (!templateSchema.version) {
      throw new BadRequestException(
        'Invalid template schema: version is required',
      );
    }

    if (!templateSchema.paperSize) {
      throw new BadRequestException(
        'Invalid template schema: paperSize is required',
      );
    }

    if (!templateSchema.orientation) {
      throw new BadRequestException(
        'Invalid template schema: orientation is required',
      );
    }

    if (!Array.isArray(templateSchema.elements)) {
      throw new BadRequestException(
        'Invalid template schema: elements must be an array',
      );
    }
    ...
    for (const element of templateSchema.elements) {
      if (!element.id || !element.type) {
        throw new BadRequestException(
          'Invalid template element: id and type are required',
        );
      }

      if (typeof element.x !== 'number' || typeof element.y !== 'number') {
        throw new BadRequestException(
          'Invalid template element: x and y must be numbers',
        );
      }
