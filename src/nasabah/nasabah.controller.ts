...
export class NasabahController {
  ...
  private async processAndSaveFile(
    ...
    try {
      ...
      console.log(
        `File saved successfully: ${filename}, format: ${isPng ? 'PNG' : 'JPEG'}, size: ${stats.size} bytes`,
      );
  ...
  async create(
    @UploadedFiles()
    files: { fileKtp?: Express.Multer.File[]; fileKk?: Express.Multer.File[] },
    ...
    return this.nasabahService
      ...
      .catch((err) => {
        ...
        throw new BadRequestException(
          err.message || 'Gagal menyimpan data nasabah',
        );
      });
  }
