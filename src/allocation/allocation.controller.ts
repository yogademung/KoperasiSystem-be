...
export class AllocationController {
  ...
  async preview(@Body() body: { ruleId: number; year: number; month: number }) {
    ...
    return this.allocationService.previewAllocation(
      body.ruleId,
      body.year,
      body.month,
    );
  ...
  @Post('execute')
  async execute(
    @Body() body: { ruleId: number; year: number; month: number },
    @Req() req: any,
  ) {
    if (!body.ruleId || !body.year || !body.month)
      throw new BadRequestException('Missing required fields');
    return this.allocationService.executeAllocation(
      body.ruleId,
      body.year,
      body.month,
      req.user.id,
  ...
  async getExecutions(
    ...
    return this.allocationService.getExecutions(
      year ? +year : undefined,
