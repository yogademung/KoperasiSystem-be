...
export class AllocationService {
  ...
  async executeAllocation(
    ...
    if (existing)
      throw new BadRequestException(
        'Allocation already executed for this period',
      );

    // 2. Preview
    const preview = await this.previewAllocation(ruleId, year, month);
    if (preview.sourceAmount <= 0)
      throw new BadRequestException('Source amount is zero or negative');
  ...
  async rollbackExecution(id: number, userId: number) {
    ...
    if (execution.status !== 'EXECUTED')
      throw new BadRequestException('Execution already rolled back');
