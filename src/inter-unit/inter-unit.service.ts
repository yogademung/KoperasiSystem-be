...
export class InterUnitService {
    ...
    async post(id: number, userId: number) {
        ...
        if (transaction.status !== 'APPROVED') {
            throw new BadRequestException('Transaction must be approved before posting');
        }
    ...
    async generateElimination(year: number, month: number, userId: number) {
        ...
        if (existing) {
            throw new BadRequestException(`Elimination already processed for ${year}-${String(month).padStart(2, '0')}`);
        }
