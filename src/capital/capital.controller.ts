...
export class CapitalController {
  ...
  createModal(@Body() dto: CreateModalDto, @Request() req) {
    ...
    return this.capitalService.createNasabahModal(
      dto,
  ...
  addTransaction(
    @Param('id') id: string,
    @Body() dto: TransModalDto,
    @Request() req,
  ) {
    return this.capitalService.addModalTransaction(
      id,
      dto,
      req.user.userId || req.user.id,
    );
  ...
  createLoan(@Body() dto: CreateExternalLoanDto, @Request() req) {
    return this.capitalService.createExternalLoan(
      dto,
      req.user.userId || req.user.id,
    );
  }
  ...
  repayLoan(
    @Param('id') id: string,
    @Body() dto: RepayExternalLoanDto,
    @Request() req,
  ) {
    return this.capitalService.repayExternalLoan(
      +id,
      dto,
      req.user.userId || req.user.id,
    );
