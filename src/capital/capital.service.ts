...
export class CapitalService {
    ...
    async repayExternalLoan(id: number, dto: RepayExternalLoanDto, userId: number) {
        return this.prisma.$transaction(async (tx) => {
            const client = tx as any;
            ...
            if (loan.schedule.length === 0) throw new BadRequestException('No upcoming installments due');
            ...
            if (Number(dto.amount) !== Number(dueInstallment.totalDue)) {
                ...
                throw new BadRequestException(`Amount must match due installment: ${dueInstallment.totalDue}`);
            }
