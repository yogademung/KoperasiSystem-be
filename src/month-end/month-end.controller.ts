...
export class MonthEndController {
  ...
  @Post('unlock-request')
  async requestUnlock(
    @Body() body: { period: string; reason: string },
    @Req() req,
  ) {
    const userId = 1;
    return this.periodLockService.requestUnlock(
      body.period,
      userId,
      body.reason,
    );
  ...
  // @Roles('MANAGER')
  async approveUnlock(
    @Body() body: { requestId: number; notes: string },
    @Req() req,
  ) {
    const managerId = 1;
    return this.periodLockService.approveUnlock(
      body.requestId,
      managerId,
      body.notes,
    );
  ...
  async adminForceUnlock(
    ...
    const adminId = 1;
    return this.periodLockService.adminForceUnlock(
      body.period,
      adminId,
      body.reason,
    );
  ...
  async postDepreciation(@Body() body: { period: string }, @Req() req) {
    const userId = 1;
    return this.depreciationService.postMonthlyDepreciation(
      body.period,
      userId,
    );
  }
