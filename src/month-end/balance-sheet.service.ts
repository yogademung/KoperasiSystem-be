...
export class BalanceSheetService {
  ...
  async validateBalance(period: string) {
    ...
    this.logger.log(
      `Balance Validation for ${period}: Assets=${totalAssets}, Liabilities=${totalLiabilities}, Equity=${totalEquity}, SHU=${currentShu}. Diff=${diff}`,
    );

    if (diff > 100) {
      // Toleransi Rp 100 perak for rounding issues
  ...
  async processRetainedEarnings(period: string, user_id: number) {
    ...
    if (mapping) {
      ...
      this.logger.warn(
        'COA Mapping for sys_RETAINED_EARNINGS not found. Using default 3.20.01',
      );
    }
    ...
    this.logger.log(
      `Year End Closing Journal Created for ${year}. SHU: ${currentSHU}`,
    );
  }
}
