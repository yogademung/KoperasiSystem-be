...
export class DepreciationService {
  ...
  async calculateMonthlyDepreciation(period: string) {
    ...
    const items: Array<{ asset: (typeof assets)[0]; amount: number }> = [];
    let totalAmount = 0;
  ...
  async postMonthlyDepreciation(period: string, userId: number) {
    ...
    if (existing) {
      throw new BadRequestException(
        `Depresiasi periode ${period} sudah diposting.`,
      );
    }

    // 2. Calculate
    const { items, totalAmount } =
      await this.calculateMonthlyDepreciation(period);
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      for (const item of items) {
        ...
        journalEntries.set(
          expAcc,
          (journalEntries.get(expAcc) || 0) + item.amount,
        );
        ...
        journalEntries.set(
          accumAcc,
          (journalEntries.get(accumAcc) || 0) - item.amount,
        );
